...
export async function sendResetPasswordEmail(email: string, name: string, resetToken: string): Promise<string> {
    ...
    <html>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #2563eb;">Welcome to ThaiChinese Tour!</h1>
          <p>Hello ${name},</p>
  ...
  await sendEmail({
    to: email,
    subject: "Set Your Password - ThaiChinese Tour",
    html,
    text,
  });
...
export async function sendForgotPasswordEmail(email: string, name: string, resetToken: string): Promise<string> {
  ...
  await sendEmail({
    to: email,
    subject: "Reset Your Password - ThaiChinese Tour",
    html,
    text,
  });
...
export async function sendEmailVerificationEmail(
  ...
  await sendEmail({
    to: newEmail,
    subject: "Verify Your New Email Address - ThaiChinese Tour",
    html,
    text,
  });
